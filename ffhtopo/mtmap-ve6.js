 ...
 * Farbe VPN-Tunnel-Link ge�ndert in grau
 * Farbe Inaktive Knoten ge�ndert in gr�n
 */
 
mtMapVe6Version = 0.10;
mtMapVe6VersionString = mtMapVe6Version + " $Rev: 25M $"
...
function mtMapVirtualEarth(layer) {
	...
	map.AttachEvent('ondoubleclick', function(e) {return self._handleEvent(e);});
	map.AttachEvent('onclick', function(e) {return self._handleEvent(e);});
	...
	this.bindInfoWindow = function(marker, callback) {
		...
		if (!boundMapInfo) {
			var self = this;
			map.AttachEvent('onmouseover', function(e) {self._prepareInfoWindow(e.elementID); return false;})
			boundMapInfo = true;
		}
	}
		...
		for (var i in events) {
			if (events[i].event == key) {
				if (events[i].object == map) {
					...
					if (e.elementID) {
						var object = map.GetShapeByID(e.elementID);
					} else {
						var object = null;
					}
